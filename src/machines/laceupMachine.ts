...
const calculateRaceAbility = async (
  context: LaceupContext,
  event: any
): Promise<String> => {
  ...
  const previousRunsNormalised = (() => {
    const aggScorePrevRaces = Object.values(previousRuns)
      .map(({ distance, time, date }) => {
        if (time === 0) {
          return 0;
        }
  ...
  const aggScoreTotal =
    (kmPerWeekNormalised + runsPerWeekNormalised + previousRunsNormalised) / 3;
  await wait(2000);
...
const hasPreviousRaces = (context: LaceupContext, _: any): boolean =>
  context.numPreviousRaces >= 3;
...
const filterRacesByAbility = (ctx: any, event: any): Array<Race> =>
  ctx.races.filter((race: Race) => race.ability === ctx.ability);

const simpleMachine = Machine<any, any, any>(
  {
    id: "laceupMachine",
    initial: "infoOne",
      ...
      raceAbility: {
        on: {
          DISPLAY_RACES_FOR_ABILITY: {
            target: "races",
            actions: assign({
              racesToDisplay: (ctx, event) => filterRacesByAbility(ctx, event),
            }),
          },
          DISPLAY_ALL_RACES: {
            target: "races",
